...
function ContainsDX_FourOP_Data_New(dmp: TMemoryStream; var StartPos: integer;
  const Report: TStrings): boolean;
...
function ContainsDX7BankDump(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
function Contains4OPVoiceDump(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
function Contains4OPBankDump(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
function RepairDX7SysEx(aFileName: string; const Report: TStrings): boolean;
function Repair4OPSysEx(aFileName: string; const Report: TStrings): boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          StartPos := dmp.Position;
          if rHeader.f = $03 then
            Report.Add('4OP Voice - VCED at position ' +
              IntToStr(StartPos));
          if rHeader.f = $04 then
            Report.Add('4OP Voice Bank - VMEM at position ' +
              IntToStr(StartPos));
          ...
          begin
            ...
            begin
              ...
              for i := Low(abLMType) to High(abLMType) do
                ...
                begin
                  case i of
                    0: Report.Add('ACED at position ' + IntToStr(StartPos));
                    1: Report.Add('ACED2 at position ' + IntToStr(StartPos));
                    2: Report.Add('ACED3 at position ' + IntToStr(StartPos));
                    3: Report.Add('PCED at position ' + IntToStr(StartPos));
    ...
    begin
      Report.Add('4OP header not found');
      Exit;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        dummy := dmp.ReadByte;                        // sub-status + channel number
        dummy := dmp.ReadByte;
        if not (dummy = $04) then StartPos := -1;     // $04 - 32 Voice dump
        dummy := dmp.ReadByte;
        if not (dummy = $20) then StartPos := -1;     // byte count MS
        dummy := dmp.ReadByte;
        if not (dummy = $00) then StartPos := -1;     // byte count LS
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (rHeader.f = $04) and (rHeader.f0 = $F0) and (rHeader.id = $43) then
        begin
          //contains data before header
          if (iDumpStart > 0) and (msToRepair.Size >= 4104 + iDumpStart) then
            needExtractDump := True;
...
begin
  ...
  begin
    ...
    begin
      msToCrop.Position := 3;
      tmpByte := msToCrop.ReadByte;
      if tmpByte = 4 then     //4OP VMEM
      begin
        msToCrop.Position := 6;
        msCropped.CopyFrom(msToCrop, 4096);
        msCropped.SaveToFile(sDirCropped + sNameCropped + '.4OPhdlsvmem.syx');
        Result := True;
      end;
    end;
    ...
    //can not crop 4OP VCED because it can contain ACEDx
  end;
